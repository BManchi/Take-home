  ...
  return (
    <SafeAreaView className="flex-1 bg-background dark:bg-background-dark">
      ...
      <View className="flex-row items-center px-4 py-2.5 gap-2.5">
        <View className="flex-1 flex-row items-center bg-input dark:bg-input-dark rounded-xl px-3">
          ...
          <TextInput
            ...
            className="flex-1 text-primary dark:text-primary-dark font-sans text-base py-2.5 ml-2"
          />
        </View>
      </View>
      ...
      {showCategoryPicker && (
        <View className="bg-surface-raised dark:bg-surface-raised-dark mx-4 mb-2 rounded-xl overflow-hidden">
              ...
              {categories.map((cat) => (
                <TouchableOpacity
                  ...
                  className={`flex-row items-center rounded-full px-3 py-1.5 ${
                    categoryIds.includes(cat.id) ? 'bg-accent' : 'bg-input dark:bg-input-dark'
                  }`}
                >
                  <Text className="text-base mr-1">{cat.emoji}</Text>
                  <Text
                    className={`font-sans-md text-sm ${
                      categoryIds.includes(cat.id) ? 'text-white' : 'text-secondary dark:text-secondary-dark'
                    }`}
                  >
                    {cat.name}
                  </Text>
          ...
          {(['regular', 'income', 'internal_transfer'] as const).map((t) => (
            <TouchableOpacity
              ...
              className={`flex-1 items-center py-1.5 rounded-full ${
                types.includes(t) ? 'bg-accent' : 'bg-input dark:bg-input-dark'
              }`}
            >
              <Text
                className={`font-sans-md text-sm ${types.includes(t) ? 'text-white' : 'text-secondary dark:text-secondary-dark'}`}
              >
        ...
        <SectionList
          ...
          ListEmptyComponent={
            <View className="items-center py-20 px-8">
              ...
              <Text className="text-primary dark:text-primary-dark font-sans-semi text-base mb-1">No transactions</Text>
              <Text className="text-secondary dark:text-secondary-dark font-sans text-sm text-center">
